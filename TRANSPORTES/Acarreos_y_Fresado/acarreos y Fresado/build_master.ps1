...
# Headers Base_Datos
$headers = @("SEMANA", "FECHA", "SINDICATO", "OBRA", "MATERIAL", "FOLIO", "PU", "SUBTOTAL", "IVA", "TOTAL")
...
function Extract-Data {
    ...
    for ($r = $startRow; $r -le $lastRow; $r++) {
        ...
        $wsBase.Cells.Item($targetRow, 3) = $sindicato
        $wsBase.Cells.Item($targetRow, 4) = $obra
        $wsBase.Cells.Item($targetRow, 5) = $material
...
# 1. CAPTURA DE NOTAS - MEZCLA (MEX-TOL)
...
# 3. MEX-TOL CARPETA (Skip because we already got MEX-TOL MEZCLA from Captura de Notas. Wait, the user said they capture in multiple fronts. The notes in CAPTURA DE NOTAS perfectly match the ones in MEX-TOL CARPETA. To avoid duplicates, I will only extract from Captura de Notas. Let's add L3M.)
...
$wsResumen.Cells.Item(3, 2).Interior.Color = 65535 # Yellow background for control cell
...
for ($i = 1; $i -le $resHeaders.Length; $i++) {
    ...
    $wsResumen.Cells.Item(5, $i).Interior.Color = 12632256 # Light gray
}

# Apply Formulas to pull from Base_Datos
# A6: UNIQUE(FILTER(Base_Datos!C:E, Base_Datos!A:A=B3, "")) (This outputs Sindicato, Obra, Material. Wait, we want Sindicato, Fecha, Material)
# Let's use UNIQUE on columns C, B, E
$wsResumen.Cells.Item(6, 1).Formula2 = "=UNIQUE(FILTER(CHOOSE({1,2,3}, Base_Datos!C:C, Base_Datos!B:B, Base_Datos!E:E), Base_Datos!A:A=B3, ""))"

# Total Viajes: =COUNTIFS(Base_Datos!A:A, B$3, Base_Datos!C:C, A6, Base_Datos!B:B, B6, Base_Datos!E:E, C6)
$wsResumen.Cells.Item(6, 4).Formula2 = "=IF(A6="", "", COUNTIFS(Base_Datos!A:A, B$3, Base_Datos!C:C, A6, Base_Datos!B:B, B6, Base_Datos!E:E, C6))"

# Importe Total: =SUMIFS(Base_Datos!J:J, Base_Datos!A:A, B$3, Base_Datos!C:C, A6, Base_Datos!B:B, B6, Base_Datos!E:E, C6)
$wsResumen.Cells.Item(6, 5).Formula2 = "=IF(A6="", "", SUMIFS(Base_Datos!J:J, Base_Datos!A:A, B$3, Base_Datos!C:C, A6, Base_Datos!B:B, B6, Base_Datos!E:E, C6))"

# Folios: =TEXTJOIN(", ", TRUE, FILTER(Base_Datos!F:F, (Base_Datos!A:A=B$3)*(Base_Datos!C:C=A6)*(Base_Datos!B:B=B6)*(Base_Datos!E:E=C6), ""))
$wsResumen.Cells.Item(6, 6).Formula2 = "=IF(A6="", "", TEXTJOIN(", ", TRUE, FILTER(Base_Datos!F:F, (Base_Datos!A:A=B$3)*(Base_Datos!C:C=A6)*(Base_Datos!B:B=B6)*(Base_Datos!E:E=C6), "")))"

# Drag formulas down to row 100 for safety
$wsResumen.Range("D6:F6").Copy() | Out-Null
$wsResumen.Range("D7:F100").PasteSpecial(-4104) | Out-Null # xlPasteAll
...
Write-Host "Archivo Maestro creado exitosamente."
