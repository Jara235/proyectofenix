...
$targetPath = $baseDir + "GC-MAT-1.0_Maestro_Consolidado_V8.xlsx"
...
function Sanitize-Obra {
    ...
    if ($v -match "MXICO" -or $v -match "MÃ‰XICO" -or $v -match "MEXICO") {
        return "MEXICO-TOLUCA" # Sin acento para evitar errores de encoding
    }
    return $val
}
...
$wsFresado = $wbTarget.Worksheets.Add( $wsMezcla ) # Adds before
$wsFresado.Name = "Dash_Fresado"

$wsTotal = $wbTarget.Worksheets.Add( $wsFresado ) # Adds before
...
# Pivot Table 1: MEZCLA (en Dash_Mezcla)
$wsMezcla.Cells.Item(1, 2) = "MEZCLA ASFÁLTICA"
$wsMezcla.Cells.Item(1, 2).Font.Bold = $true
$pt1 = $pc.CreatePivotTable("Dash_Mezcla!R3C2", "PT_Mezcla")
...
# Pivot Table 2: FRESADO (en Dash_Fresado)
$wsFresado.Cells.Item(1, 2) = "FRESADO"
$wsFresado.Cells.Item(1, 2).Font.Bold = $true
$pt2 = $pc.CreatePivotTable("Dash_Fresado!R3C2", "PT_Fresado")
...
Write-Host "Archivo Maestro final (Tablas Dinamicas V8) creado."
