...
$destPath = $baseDir + "GC-MAT-1.0_Maestro_Consolidado_V14.xlsx"

# Leer desde V12 actual (la base de datos perfecta)
$wbSrc = $excel.Workbooks.Open($srcPath)
$wsSrc = $wbSrc.Worksheets.Item("Base_Datos")
$lastRow = $wsSrc.UsedRange.Rows.Count
Write-Host "Copiando $lastRow filas de la Base de Datos perfecta"
...
# Convertir a Excel Table para que JAMAS vuelva a fallar por rango
...
$listObj = $wsBase.ListObjects.Add(1, $wsBase.Range("A1:O" + $lastRow), 0, 1)
$listObj.Name = "TablaDatosOFICIAL"
$listObj.TableStyle = "TableStyleMedium9"

# Crear cache apuntando a la tabla oficial
$pc = $wbNew.PivotCaches().Create(1, "TablaDatosOFICIAL", 6)
...
Write-Host "V14 CREADA EXITOSAMENTE"
