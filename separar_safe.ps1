...
$currentRow = 2
for ($r = 2; $r -le 1000; $r++) {
    $cellValue = $sheetBD.Cells.Item($r, 5).Value2
    if ($null -ne $cellValue) {
        $movType = [string]$cellValue
...
# Apply sort only to rows that have data to push empty rows down
# B column is Fecha.
# En COM PowerShell a veces Sort() da error si no se declara bien. Lo omitimos, le pediremos al usuario que ordene.
...
Write-Host "Separacion de solicitudes completada exitosamente sin errores."
