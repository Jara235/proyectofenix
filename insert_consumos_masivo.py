...
registros_lerma = [
    ...
    ('2026-07-13', 'NEUMATICO PF-300B', 75.0),
    ('2026-07-13', 'RETROEXCAVADORA JHONDEREE 310 K', 50.0),
    ('2026-07-13', 'BARREDORA LAYMOR SM400', 20.0),
    ('2026-07-13', 'COMPRESOR SULLIVAN PALATEK D2010', 59.0),
...
for idx, (fecha, equipo, litros) in enumerate(registros_lerma):
    folio = f'GC-COMB-006-SEM29-LERMA-{idx+1:02d}'
...
for idx, (fecha, equipo, litros) in enumerate(registros_colegio):
    folio = f'GC-COMB-006-SEM29-COLMIL-{idx+1:02d}'
    db.execute('''INSERT INTO diesel.consumos (folio_conciliacion, fecha, semana, origen, obra_destino, equipo, litros, importe_total, operador, estatus_revision, tipo_movimiento)
        VALUES (%s, %s, %s, %s, %s, %s, %s, 0.0, %s, %s, 'Salida')''', 
...
registros_alfredo = [
    ...
    ('2026-07-13', 'BARREDORA BROCE BROOM KR350', 40.0),
    ('2026-07-13', 'RETROEXCAVADORA JHONDEREE 310SK', 60.0),
    ('2026-07-13', 'PERFILADORA RENTADA', 400.0),
    ('2026-07-13', 'PETROLIZADORA (IMPACTO 04) FORD', 60.0),
    ('2026-07-13', 'PAVIMENTADORA VOGELE 1800-3 I', 160.0),
    ('2026-07-13', 'DOBLE RODILLO/VIBROCOMP', 60.0),
    ('2026-07-13', 'NEUMATICO / COMPACTADOR DE', 30.0),
...
for idx, (fecha, equipo, litros) in enumerate(registros_alfredo):
    folio = f'GC-COMB-006-SEM29-ALFREDO-{idx+1:02d}'
    db.execute('''INSERT INTO diesel.consumos (folio_conciliacion, fecha, semana, origen, obra_destino, equipo, litros, importe_total, operador, estatus_revision, tipo_movimiento)
        VALUES (%s, %s, %s, %s, %s, %s, %s, 0.0, %s, %s, 'Salida')''', 
