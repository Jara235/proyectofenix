...
def api_semanas():
    ...
    cur.execute('''
        SELECT DISTINCT semana FROM fenix_movimientos_combustible WHERE semana IS NOT NULL
        UNION
        SELECT DISTINCT semana FROM fenix_gas_tickets_reales WHERE semana IS NOT NULL
        UNION
        SELECT DISTINCT semana FROM fenix_gas_presupuestos WHERE semana IS NOT NULL
...
# ─────────────────────────────────────────────────
@app.route("/api/gasolina/tendencia")
def api_gasolina_tendencia():
    db = get_db(); cur = db.cursor()
    cur.execute("""SELECT semana, ROUND(SUM(litros),1) AS litros, ROUND(SUM(importe),2) AS importe
...
def api_gasolina_kpis():
    db = get_db(); cur = db.cursor()
    sem = semana_param()
    PRECIO_LEVET = 22.89  # MXN por litro — precio promedio Levet
    ...
    importe_aut = aut["importe_aut"] or 0
    litros_autorizados = round(importe_aut / PRECIO_LEVET, 1) if importe_aut else 0
    
    db.close()
    return jsonify({
        "importe_autorizado": importe_aut,
...
@app.route("/api/gasolina/conciliacion_por_placa")
def api_gasolina_conciliacion_por_placa():
    """Resumen de presupuesto vs consumo real agrupado por placa y semana."""
    db = get_db(); cur = db.cursor()
    sem = semana_param()

    # Presupuestos por placa
    sql_pres = """
        SELECT semana, responsable, obra, unidad, placa,
               ROUND(monto_autorizado, 2) AS importe_autorizado
        FROM fenix_gas_presupuestos
        WHERE placa IS NOT NULL AND placa != '' AND placa != 'NO REPORTADA'
    """
    sql_pres, p_pres = q_filter(sql_pres, "semana", sem)
    cur.execute(sql_pres, p_pres)
    presupuestos = [dict(r) for r in cur.fetchall()]

    # Consumo real por placa y semana
    sql_tick = """
        SELECT placa, semana,
               ROUND(SUM(litros), 3) AS litros_consumidos,
               ROUND(SUM(importe), 2) AS importe_consumido
        FROM fenix_gas_tickets_reales
        WHERE placa IS NOT NULL AND placa != '' AND placa != 'NO REPORTADA'
    """
    sql_tick, p_tick = q_filter(sql_tick, "semana", sem, False)
    cur.execute(sql_tick + " GROUP BY placa, semana", p_tick)
    tickets_por_placa = {(r['placa'], r['semana']): dict(r) for r in cur.fetchall()}

    PRECIO_LEVET = 22.89
    resultado = []
    for p in presupuestos:
        key = (p['placa'], p['semana'])
        tick = tickets_por_placa.get(key, {'litros_consumidos': 0, 'importe_consumido': 0})
        lts_aut = round(p['importe_autorizado'] / PRECIO_LEVET, 1)
        lts_real = tick['litros_consumidos']
        diferencia = round((tick['importe_consumido'] or 0) - p['importe_autorizado'], 2)
        resultado.append({
            'placa': p['placa'],
            ...
            'unidad': p['unidad'],
            'semana': p['semana'],
            'litros_autorizados': lts_aut,
            'litros_consumidos': lts_real,
            'importe_autorizado': p['importe_autorizado'],
            'importe_consumido': tick['importe_consumido'] or 0,
            'diferencia': diferencia
        })

    resultado.sort(key=lambda x: abs(x['diferencia']), reverse=True)
    db.close()
    return jsonify(resultado)

@app.route("/api/gasolina/tickets_desconocidos")
def api_gasolina_tickets_desconocidos():
    """Tickets donde placa='NO REPORTADA', filtrados por semana."""
    db = get_db(); cur = db.cursor()
    sem = semana_param()
    sql = """
        SELECT ticket, fecha, semana, placa, conductor, responsable, obra, unidad,
               ROUND(litros, 3) AS litros, ROUND(precio_litro, 4) AS precio_litro,
               ROUND(importe, 2) AS importe, gasolinera, folio
        FROM fenix_gas_tickets_reales
        WHERE placa = 'NO REPORTADA'
    """
    sql, p = q_filter(sql, "semana", sem)
    cur.execute(sql + " ORDER BY fecha DESC, ticket", p)
    rows = [dict(r) for r in cur.fetchall()]
    db.close()
    return jsonify(rows)
...
def api_totales():
    ...
    sql_fac = """SELECT tipo_combustible,
                          COUNT(*) as facturas,
                          ROUND(SUM(litros_totales),1) as litros,
                          ROUND(SUM(total),2) as monto
                   FROM fenix_facturas_documentos
                   WHERE estatus_validacion='VALIDADA'"""
    sql_fac, p_fac = q_filter(sql_fac, "semana", sem, True)
    sql_fac += " GROUP BY tipo_combustible"
    cur.execute(sql_fac, p_fac)
