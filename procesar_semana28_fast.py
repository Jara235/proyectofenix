...
def main():
    ...
    try:
        print("Cargando workbook (esto toma unos segundos)...")
        wb = openpyxl.load_workbook(excel_path)
        print("Workbook cargado.")
        sheet = wb["BD_FACTURAS"]
        
        # Buscar el final real basándonos en la columna B (Folio)
        print("Buscando ultima fila con datos en BD_FACTURAS...")
        last_row = 1
        folios_existentes = set()
        while True:
            val = sheet.cell(row=last_row + 1, column=2).value
            if val is None:
                # Comprobar si hay 5 seguidos vacíos para estar seguros de que terminó
                any_more = False
                for check in range(2, 6):
                    if sheet.cell(row=last_row + check, column=2).value is not None:
                        any_more = True
                        break
                if not any_more:
                    break
        ...
        print(f"Ultima fila real encontrada: {last_row}")
        
        agregados = 0
        for d in facturas_extraidas:
            if str(d['folio']).strip() in folios_existentes:
                print(f"Factura folio {d['folio']} ya existe, omitiendo.")
                continue
                
            last_row += 1
            # Col 2 (B): FOLIO
            sheet.cell(row=last_row, column=2).value = d['folio']
            # Col 3 (C): FECHA
            sheet.cell(row=last_row, column=3).value = d['fecha']
            # Col 5 (E): PROVEEDOR
            sheet.cell(row=last_row, column=5).value = d['proveedor']
            # Col 6 (F): PUNTO DE CARGA
            sheet.cell(row=last_row, column=6).value = d['destino']
            # Col 7 (G): LITROS
            sheet.cell(row=last_row, column=7).value = d['litros']
            # Col 8 (H): PRECIO
            sheet.cell(row=last_row, column=8).value = d['precio']
            # Col 9 (I): IMPORTE
            sheet.cell(row=last_row, column=9).value = d['importe']
            # Col 10 (J): IVA
            sheet.cell(row=last_row, column=10).value = d['iva']
            # Col 11 (K): TOTAL
            sheet.cell(row=last_row, column=11).value = d['total']
            # Col 12 (L): TIPO
            sheet.cell(row=last_row, column=12).value = "Diésel"
            
            # Formulas
            sheet.cell(row=last_row, column=4).value = f'=IF(C{last_row}="","", "Semana "&WEEKNUM(C{last_row},2))'
            sheet.cell(row=last_row, column=1).value = f'=IF(C{last_row}="","", "FA" & "-" & IFERROR(VLOOKUP(F{last_row},CATALOGOS!$A$2:$B$50,2,0),"XX") & "-" & WEEKNUM(C{last_row},2) & "-" & TEXT(ROW()-1,"000"))'
            agregados += 1
            
        if agregados > 0:
            print(f"Guardando Excel con {agregados} nuevas facturas...")
            wb.save(excel_path)
