...
# Wait up to 30s for the file to be available
for i in range(30):
    try:
        with open(maestro_path, 'r+b') as f:
            pass
        print('File is available, building sheet...')
        break
    except PermissionError:
        print(f'  waiting... ({i+1}s)')
        time.sleep(1)
else:
    print('ERROR: File still locked after 30s. Please close Excel.')
    exit(1)
...
headers = ['SEMANA','GASOLINERA','REPORTES (Regs)','REPORTES (Lts)','REPORTES (Importe)',
           'FACTURAS (Regs)','FACTURAS (Lts)','FACTURAS (Importe)',
...
print('OK - CONCILIACION_GASOLINERAS sheet created!')
print(df.to_string())
