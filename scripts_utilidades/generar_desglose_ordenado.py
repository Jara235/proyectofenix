...
# Script para generar el ordenamiento completo por semana
tabla_1_raw = [
...
registros = []
for f in tabla_1_raw:
    dt = parse_date(f[0])
    sem = get_week(dt)
    registros.append({
        'tabla': 'Tabla 1 ($747k)', 'fecha': dt, 'fecha_str': f[0], 'importe': f[1],
        'centro': f[2], 'folio': f[3], 'semana': sem
    })

for f in tabla_2_raw:
    dt = parse_date(f[0])
    sem = get_week(dt)
    registros.append({
        'tabla': 'Tabla 2 ($915k)', 'fecha': dt, 'fecha_str': f[0], 'importe': f[1],
        'centro': f[2], 'folio': f[3], 'semana': sem
    })

# Ordenar por Semana, Fecha, Centro, Folio
registros_ordenados = sorted(registros, key=lambda x: (x['semana'], x['fecha'], x['centro'], x['folio']))

print(f"Total registros: {len(registros_ordenados)}")
for sem in [28, 29, 30, 31]:
    sub = [r for r in registros_ordenados if r['semana'] == sem]
    tot_sem = sum(r['importe'] for r in sub)
    print(f"\nSEMANA {sem} ({len(sub)} partidas, Total: ${tot_sem:,.2f}):")
    for r in sub:
        print(f"  {r['fecha']} | {r['folio']:<12} | ${r['importe']:>10,.2f} | {r['centro']}")
