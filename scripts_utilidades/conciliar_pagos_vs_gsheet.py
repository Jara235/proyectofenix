...
# Cargar Google Sheet descargado
...
def clean_fol(f):
    return re.sub(r'[^0-9]', '', str(f))

# Mapear facturas pagadas
pagadas_map = {}
todas_pagadas = []
for f in tabla_1_raw + tabla_2_raw:
    ...
    cf = clean_fol(f[3])
    item = {
        'fecha': dt, 'fecha_str': f[0], 'importe': f[1], 'centro': f[2], 'folio': f[3], 'folio_clean': cf, 'semana': sem
    }
    todas_pagadas.append(item)
    if cf not in pagadas_map:
        pagadas_map[cf] = []
    pagadas_map[cf].append(item)

# Analizar las facturas del Estado de Cuenta en Google Sheets
ec_semanas = {
    28: {'gasolina': [], 'diesel': [], 'total_edo': 334052.62},
    29: {'gasolina': [], 'diesel': [], 'total_edo': 391703.36},
    30: {'gasolina': [], 'diesel': [], 'total_edo': 410312.10},
    31: {'gasolina': [], 'diesel': [], 'total_edo': 384745.72},
    32: {'gasolina': [], 'diesel': [], 'total_edo': 341114.27},
}

# Extraer de Estado de Cuenta los strings de facturas
for r in range(31, ws_ec.max_row + 1):
    val_a = str(ws_ec.cell(r, 1).value or '').strip()
    val_b = str(ws_ec.cell(r, 2).value or '').strip()
    val_c = str(ws_ec.cell(r, 3).value or '').strip()
    val_d = str(ws_ec.cell(r, 4).value or '').strip()
    
    # Check week headers
    m_sem = re.search(r'Semana\s+(\d+)', val_a)
    if m_sem:
        curr_sem = int(m_sem.group(1))

print("=== CONCILIACIÓN SEMANA POR SEMANA ===")
# Para cada semana comparar lo pagado en imágenes vs lo listado en Estado de Cuenta
for sem in [28, 29, 30, 31, 32]:
    pag_sem = [p for p in todas_pagadas if p['semana'] == sem]
    tot_pag = sum(p['importe'] for p in pag_sem)
    tot_edo = ec_semanas.get(sem, {}).get('total_edo', 0.0)
    print(f"\n>> SEMANA {sem}:")
    print(f"   * Pagado en Imágenes: {len(pag_sem)} partidas | Total: ${tot_pag:,.2f}")
    print(f"   * Registrado en Edo Cta: Total: ${tot_edo:,.2f}")
    print(f"   * Diferencia: ${tot_pag - tot_edo:,.2f}")
