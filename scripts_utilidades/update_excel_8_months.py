...
# Replace sheet
if "Comparativo por Obra (8 Meses)" in wb.sheetnames:
    del wb["Comparativo por Obra (8 Meses)"]
if "Comparativo por Obra" in wb.sheetnames:
    del wb["Comparativo por Obra"]

ws_comp = wb.create_sheet(title="Comparativo por Obra (8 Meses)", index=0)
...
ws_comp['A1'] = "📊 COMPARATIVO DE 8 MESES POR OBRA: ESTIMACIÓN Y REAL VS VALOR DE REFERENCIA"
...
ws_comp['A2'] = "Periodo Completo: Enero a Agosto | Obras México - Toluca y Lerma - Tres Marías | Fénix 2.0"
...
ws_comp['A4'] = "📍 1. OBRA: MÉXICO - TOLUCA (VALOR DE REFERENCIA MENSUAL: $334,486.40 MXN | 12,583.8 LITROS)"
...
data_mt = [
    ...
    ("Julio", 11691.0, 1882.6, 13573.6, 303320.01, 334486.40, -31166.39, -9.32, "NO", "Mes completo cerrado (5 semanas): Operación activa constante regulada por lluvias intermitentes de verano."),
    ("Agosto", 11980.0, 1814.0, 13794.0, 366814.60, 334486.40, 32328.20, 9.66, "SÍ", "Ventana de Canícula (días secos a mediados de mes) aprovechada para intensificar tiros de asfalto y bacheo profundo.")
]
...
# Total México - Toluca
ws_comp.cell(row=r_mt, column=1, value="TOTAL MÉXICO - TOLUCA (8 MESES)").fill = fill(BLUE_HDR)
...
tot_mt_d = sum(d[1] for d in data_mt)
tot_mt_g = sum(d[2] for d in data_mt)
tot_mt_v = sum(d[3] for d in data_mt)
tot_mt_imp = sum(d[4] for d in data_mt)
ref_mt_tot = 334486.40 * 8

ws_comp.cell(row=r_mt, column=2, value=tot_mt_d).number_format = '#,##0.0'
ws_comp.cell(row=r_mt, column=3, value=tot_mt_g).number_format = '#,##0.0'
ws_comp.cell(row=r_mt, column=4, value=tot_mt_v).number_format = '#,##0.0'
ws_comp.cell(row=r_mt, column=5, value=tot_mt_imp).number_format = '"$"#,##0.00'
ws_comp.cell(row=r_mt, column=6, value=ref_mt_tot).number_format = '"$"#,##0.00'

ws_comp.cell(row=r_mt, column=7, value=tot_mt_imp - ref_mt_tot).number_format = '"+"#,##0.00;"-"#,##0.00;"$0.00"'
ws_comp.cell(row=r_mt, column=8, value=(tot_mt_imp - ref_mt_tot) / ref_mt_tot).number_format = '+0.0%;-0.0%;0.0%'
ws_comp.cell(row=r_mt, column=9, value="3 de 8").fill = fill(BLUE_HDR)
...
ws_comp.cell(row=r_mt, column=10, value="Presupuesto 8 Meses México-Toluca: $2.58M vs $2.67M de Referencia (-3.4%).").fill = fill(BLUE_HDR)
...
ws_comp[f'A{r_mt}'] = "📍 2. OBRA: LERMA - TRES MARÍAS (VALOR DE REFERENCIA MENSUAL: $258,291.50 MXN | 9,765.9 LITROS)"
...
data_lt = [
    ...
    ("Febrero", 7080.6, 1491.7, 8572.3, 226827.83, 258291.50, -31463.67, -12.18, "NO", "Buen avance en tramos de terracerías, base y bacheo, pero limitado por los 28 días naturales del mes (20 días de operación)."),
    ...
    ("Mayo", 10620.9, 2221.2, 12842.1, 339850.98, 258291.50, 81559.48, 31.58, "SÍ", "MES PICO: Máxima intensidad para proteger terracerías y colocar carpeta corrida antes del temporal en la sierra."),
    ...
    ("Julio", 11360.3, 1980.2, 13340.5, 250327.45, 258291.50, -7964.05, -3.08, "NO", "Mes completo cerrado: Alto volumen en fresado (Semana 29 pico) compensado con días de lluvia intensa en Semanas 28 y 30."),
    ("Agosto", 7626.0, 1626.0, 9252.0, 244763.40, 258291.50, -13528.10, -5.24, "NO", "Lluvias persistentes en la parte alta de la sierra de Tres Marías; frentes intermitentes de bacheo y riego de impregnación.")
]
...
ws_comp.cell(row=r_mt, column=1, value="TOTAL LERMA - TRES MARÍAS (8 MESES)").fill = fill(TEAL_HDR)
ws_comp.cell(row=r_mt, column=1).font = font(bold=True, color=WHITE, size=9); ws_comp.cell(row=r_mt, column=1).alignment = align('center', 'center')

tot_lt_d = sum(d[1] for d in data_lt)
tot_lt_g = sum(d[2] for d in data_lt)
tot_lt_v = sum(d[3] for d in data_lt)
tot_lt_imp = sum(d[4] for d in data_lt)
ref_lt_tot = 258291.50 * 8

ws_comp.cell(row=r_mt, column=2, value=tot_lt_d).number_format = '#,##0.0'
ws_comp.cell(row=r_mt, column=3, value=tot_lt_g).number_format = '#,##0.0'
ws_comp.cell(row=r_mt, column=4, value=tot_lt_v).number_format = '#,##0.0'
ws_comp.cell(row=r_mt, column=5, value=tot_lt_imp).number_format = '"$"#,##0.00'
ws_comp.cell(row=r_mt, column=6, value=ref_lt_tot).number_format = '"$"#,##0.00'

ws_comp.cell(row=r_mt, column=7, value=tot_lt_imp - ref_lt_tot).number_format = '"+"#,##0.00;"-"#,##0.00;"$0.00"'
ws_comp.cell(row=r_mt, column=8, value=(tot_lt_imp - ref_lt_tot) / ref_lt_tot).number_format = '+0.0%;-0.0%;0.0%'
ws_comp.cell(row=r_mt, column=9, value="2 de 8").fill = fill(TEAL_HDR)
ws_comp.cell(row=r_mt, column=9).font = font(bold=True, color=WHITE, size=8); ws_comp.cell(row=r_mt, column=9).alignment = align('center', 'center')

ws_comp.cell(row=r_mt, column=10, value="Presupuesto 8 Meses Lerma-Tres Marías: $1.97M vs $2.06M de Referencia (-4.4%).").fill = fill(TEAL_HDR)
ws_comp.cell(row=r_mt, column=10).font = font(italic=True, color='94A3B8', size=8); ws_comp.cell(row=r_mt, column=10).alignment = align('left', 'center')
...
wb.save(filename)
print("Updated Excel file with 8-month per-obra comparison tables successfully!")
