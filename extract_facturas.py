...
# Map to store counts for folio generation
week_counts = {}

for root_dir, dirs, files in os.walk(gasolina_dir):
    # Determine week from folder name
    ...
    for f in files:
        if f.lower().endswith('.xml'):
            ...
            try:
                ...
                if not folio_xml:
                    # Try to use UUID prefix if no folio
                ...
                # Try to map PUNTO_DE_CARGA from provider
                ...
                if conceptos is not None:
                    for concepto in conceptos.findall('cfdi:Concepto', ns):
                        ...
                        if 'GASOLINA' in desc or 'MAGNA' in desc or 'PREMIUM' in desc or clave in ('15101514', '15101515'):
                            ...
                            importe_total += importe_concepto
                            precio_unitario = valor_unitario # Approximation if multiple
                            ...
                            if impuestos is not None:
                                ...
                                if traslados is not None:
                                    for traslado in traslados.findall('cfdi:Traslado', ns):
                                        if traslado.attrib.get('Impuesto') == '002': # 002 is IVA
                ...
                if litros_total > 0:
                    ...
                    total_factura = importe_total + iva_total
                    ...
                    facturas_data.append({
                        ...
                        'LITROS_FACTURADOS': round(litros_total, 2),
...
print(f"Extracted {len(df_facturas)} gasolina invoices into BD_FACTURAS.")
for s, count in week_counts.items():
    if count > 0:
        print(f"  Semana {s}: {count} invoices")
