...
try:
    ...
    for index, row in df.iterrows():
        ...
        if basename in file_map:
            for actual_filename in list(file_map[basename]):
                ...
                if os.path.exists(src):
                    try:
                        shutil.move(src, dst)
                        moved_count += 1
                        file_map[basename].remove(actual_filename)
                    except Exception as ex:
                        locked_files.append(actual_filename)
                        
    print(f"Exito: Se clasificaron y movieron {moved_count} archivos (XML y PDF).")
    
    if locked_files:
        print(f"ATENCION: No se pudieron mover {len(locked_files)} archivos porque estan abiertos/bloqueados: {locked_files[:3]}...")
